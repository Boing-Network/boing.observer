...
- There is **no** second-RPC failover on **`Method not found`**; the QA page surfaces **canonical JSON/docs** when the registry RPC call fails. HTTP **530/5xx** on the public hostname **does** fail over to hosted Fly RPC (`boing-testnet-1.fly.dev`, then `-2`).
...
- **Same RPC snapshot for DEX listing:** **`boing_getNetworkInfo`** should be read as one snapshot with **`developer.dex_discovery_methods`** and **`end_user.canonical_native_dex_factory`** together; **`boing_listDexPools`** / **`boing_listDexTokens`** are factory-scoped and apply to that same node view. In this repo, factory resolution goes through **`resolveNativeDexFactoryForExplorer`** (`src/lib/resolve-native-dex-factory.ts`: live `rpc_end_user` hints or host env overrides only — historical SDK-embedded hexes are ignored until the hosted chain publishes a factory). **`/dex/tokens`** uses that resolver. **`/tokens`** (block-scan index) uses the same live-only factory for `register_pair` merge and lists **included** `ContractDeploy*` txs even when receipts are missing (skip only `success === false`).
- **Finance deploy vs explorer listing:** `boing.finance` toasts **Deploy submitted** on mempool accept (`boing_submitTransaction` → `{ tx_hash: "ok" }`). Hosted Fly at last smoke had **no** `ContractDeploy` txs (only faucet Transfers) and **null** canonical factory, so both `/tokens` and `/dex/tokens` were empty. Public gateway now fans out submits to validator + full node so deploys are not stranded on `boing-testnet-2`.
...
**Last public-RPC smoke (handwritten in repo):** 2026-08-24 — hosted Fly testnet via `https://testnet-rpc.boing.network/`; `boing_getNetworkInfo` **head_height 5**; `end_user.canonical_native_*` all **null** (DEX not bootstrapped). Blocks 1–5 are faucet **Transfer** only (no `ContractDeploy`). Explorer `/api/rpc` failsover to `boing-testnet-1.fly.dev` / `-2` on HTTP 5xx/530.
...
- `/tokens`
  - Block-scan token / asset index (included `ContractDeploy*` + live-factory `register_pair`).
- `/dex/tokens`
