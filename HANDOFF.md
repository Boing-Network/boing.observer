...
**Last public-RPC smoke (handwritten in repo):** 2026-08-24 — hosted Fly testnet via `https://testnet-rpc.boing.network/`; `boing_getSyncState` **head_height 3**; `end_user.canonical_native_*` all **null** (DEX not bootstrapped on this ledger). Explorer `/api/rpc` failsover to `boing-testnet-1.fly.dev` / `-2` on HTTP 5xx/530.
